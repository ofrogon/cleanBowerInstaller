...
function FileObj(config) {
	...
	this.bowerFileFolder = path.join(process.cwd(), (config.bowerFileFolder || "bower_components"));
		...
		for (var i = 0; i < length; i++) {
			...
			if (regex.startWithSlash.test(fileFolder)) {
				...
				unCleanList[action].push({
					"from": f,
					"to": path.join(this.bowerFileFolder, fileFolder.substr(1), fileName + path.extname(f))
				});
			} else if (regex.startWithSlash.test(libFolder)) {
				// The specified lib folder is global
				unCleanList[action].push({
					"from": f,
					"to": path.join(this.bowerFileFolder, libFolder.substr(1), fileFolder, fileName + path.extname(f))
				});
			} else {
				var df;
				...
				unCleanList[action].push({
					"from": f,
					"to": path.join(this.bowerFileFolder, df, extFolder, libFolder, fileFolder, fileName + path.extname(f))
				});
			}
		}
