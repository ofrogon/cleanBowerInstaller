 ...
 */
function arrayOfGlob(globs: string[], bowerFileFolder: string, libName: string, callback: Function) {
	glob(path.join(bowerFileFolder, libName, globs.pop()), (err, data) => {
		if (err) {
			callback(err, null);
		} else {
			if (globs.length > 0) {
				arrayOfGlob(globs, bowerFileFolder, libName, (err, cData) => {
					if (err) {
						callback(err, null);
					} else {
...
class FileObj {
	...
	getList(callback: Function) {
		...
		for (let libs in src) {
			if (src.hasOwnProperty(libs)) {
				...
				promises.push((libs, uncleanList) => {
					return this.enumeratePackages(libs, libName, libFolder, (err, data) => {
						...
						promises.push(new Promise((resolve, reject) => {
							arrayOfGlob([fileNameAndExt], this.bowerFileFolder, libName, (err, ans) => {
								if (err) {
									...
									const result = this.enumerateFile(ans.data, fileName, libFolder, fileFolder, ans === "!" ? "ignore" : "move");
	 ...
	 */
	enumerateFile(files: string[], fileName: string, libFolder: string, fileFolder: string, action: string) {
		let f;
		let unCleanList = {
			ignore: [],
			move: []
	 ...
	 * @returns {Array}
	 */
	static clean(unCleanList): string[] {
		let list = [];

		if (Object.keys(unCleanList).length >= 0) {
			const length = unCleanList.ignore.length;
			let length2 = unCleanList.move.length;
		...
		this.getList((err, list) => {
			if (err) {
				...
				for (let i = 0, length = list.length; i < length; i++) {
					promises.push(new Promise((resolve, reject) => {
						fse.copy(list[i].from, list[i].to, (err) => {
							if (err) {
								reject(err);
							} else {
								resolve();
 ...
 * @param callback {Function}
 */
const moveFiles = (config, callback: Function) => {
 ...
 * @param callback {Function}
 */
const moveFilesAndRemove = (config, callback: Function) => {
	const list = new FileObj(config);
